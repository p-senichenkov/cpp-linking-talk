...
\begin{frame}{ABI}
    \begin{itemize}
        \item Itanium C++ ABI\footnote{\url{https://itanium-cxx-abi.github.io/cxx-abi/abi.html}}
    \end{itemize}
\end{frame}
...
\begin{frame}{Виртуальные функции}
    \begin{itemize}
        \item \comment{TODO: виртуальные функции в контексте линковки}
...
\begin{frame}{LTO}
    \begin{itemize}
        ...
        \item Link-time optimizer читает промежуточное представление и работает вместе с линкером
            \begin{itemize}
                \item Линкер подключает \texttt{liblto\textunderscore plugin.so} или \texttt{libLTO.so}
            \end{itemize}
        \pause
...
\begin{frame}{Линковка в Rust}
    \comment{Основное отличие от \cpp в том, что основная единица сборки "--- это crate, а не
    translation unit.
    Соответственно, компилятор больше знает о структуре программы.}
\end{frame}

\begin{frame}[fragile]{Name mangling и ABI}
    \comment{Тоже есть mangling, и ABI тоже нестабильный.
    Тоже есть pub extern ``C''.}

    \comment{Чтобы выставить функцию из Rust наружу, используем}

    \begin{minted}[autogobble=true]{rust}
        #[unsafe(no_mangle)]
...
\begin{frame}{Линковка в Go}
    \comment{А что тут говорить?
    В целом, ситуация примерно та же, что и в rust.}
\end{frame}
...
\begin{frame}[fragile]{Системы сборки для Rust и go}
    \comment{Специфичны для конкретного языка.
    Больше знают о структуре программы.
    Нет бойлерплейта с указанием зависимостей "--- могут найти по коду.}
\end{frame}
